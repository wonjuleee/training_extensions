...
class ImageFilePathCheck(FilePathCheck):
    """Class to check image file path parameters"""
...
class MvtecDirCheck:
    """Class to check optional MvtecDataset directory path parameter"""
    ...
    def check(self):
        ...
        if isinstance(self.parameter, Path):
            parameter_str = self.parameter.__str__()
        else:
            parameter_str = self.parameter
        check_that_parameter_is_not_empty(
            parameter=parameter_str, parameter_name=self.parameter_name
        )
        check_that_null_character_absents_in_string(
            parameter=parameter_str, parameter_name=self.parameter_name
        )
